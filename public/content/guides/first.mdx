...
- Avoid lightweight OSs for now as many don't ship with graphical user interfaces. Likewise, make sure your distribution has a graphical installer and configuration program (using otherwise is not advised).
...
- The most broad OSs are also the most welcoming. Try <Link href="/d/fedora">Fedora</Link> or <Link href="/d/ubuntu">Ubuntu</Link>. These share reliability, the easy to use GNOME desktop, are easy and fast to install and setup, and have widely accessible documentation. Fedora instantly supports easy installation of popular programs like Spotify and Discord using the Flatpak system (which is integrated with other repositories). Use Ubuntu if you need maximum hardware support.
